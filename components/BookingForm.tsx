...
const BookingForm: React.FC<BookingFormProps> = ({ room, onBookingSuccess, onCancel }) => {
  ...
  useEffect(() => {
    // load rooms to allow quick preview even without search, unless a specific room is passed
    if (!room) {
  ...
  // If a specific room is passed, use it, otherwise show search results or suggestions
  const roomsToShow = room ? [room] : (availableRooms.length > 0 ? availableRooms : allRooms);
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4 items-center flex-wrap">
        ...
        {!room && ( // Hide search button if we are booking a specific room
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-80 overflow-y-auto pr-2">
          {roomsToShow.map((r) => {
            ...
            const isBookable = (room || availableRooms.some(ar => ar.id === r.id)) && nights > 0;
